...
from img_process import lib_color_change, skin_color_change, eye_color_change, nose_color_change, brow_color_change
...
def inference(request):
    ...
    if request_json['type'] == 'inference':
    ...
    elif request_json['type'] == 'enhance_brow':
        img = request_json['data']
        img = base64.b64decode(img)
        img = np.frombuffer(img, dtype=np.uint8)
        img = cv2.imdecode(img, cv2.IMREAD_COLOR)

        cv2.imwrite('debug_enhance_brow/0_org.jpg', img)
        ...
        cv2.imwrite('debug_enhance_brow/1_segment.jpg', np.uint8(label_visualize(segment, 19) * 255.0))

        value = request_json['value']
        value.reverse()
        img_result = brow_color_change(img.copy(), segment, value)

        cv2.imwrite('debug_enhance_brow/2_result.jpg', img_result)
